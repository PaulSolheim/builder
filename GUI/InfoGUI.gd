...
func _ready() -> void:
	...
	# Make sure that the entity is out of the way when the game first starts.
	hide()
...
func _set_info(entity: Node) -> void:
	...
	label.text = output
	# We finally the the tooltip visible as it now has text to display.
	show()
...
func _on_hovered_over_entity(entity: Node) -> void:
	# Keep up to date on which entity is currently being updated.
